...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        if not self.test or self.dataset == "ImageNet":
            prompts = torch.split(prompts, [len(v) for v in self.class_descriptor.values()], dim=0)
            tokenized_prompts = torch.split(tokenized_prompts, [len(v) for v in self.class_descriptor.values()], dim=0)
