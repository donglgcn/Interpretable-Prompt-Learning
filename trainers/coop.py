...
class PromptLearner_descriptor(nn.Module):
    def __init__(self, cfg, class_descriptors, clip_model):
        ...
        len_list = [1/len(v) for v in class_descriptors.values() for i in range(len(v))]
        weights = torch.tensor(len_list, dtype=dtype)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        if not self.test and self.dataset == "ImageNet":
            ...
            tokenized_prompts_new = [tokenized_prompt[indices[i]] for i, tokenized_prompt in enumerate(tokenized_prompts)]
        ...
        # random sample 4 text descriptors to train in order to deal with the OOM issue.
        if not self.test and self.dataset == "StanfordCars":
            prompts = torch.split(prompts, len_list, dim=0)
            weights = torch.split(weights, len_list)
            tokenized_prompts = torch.split(tokenized_prompts, len_list, dim=0)
            indices = [torch.randperm(len(prompt))[:5] for prompt in prompts]
            prompts_new = [prompt[indices[i]] for i, prompt in enumerate(prompts)]
            weights_new = [weight[indices[i]] for i, weight in enumerate(weights)]
            ...
            weights = torch.cat(weights_new)
            tokenized_prompts = torch.cat(tokenized_prompts_new, dim=0)
            len_list = [5 for v in self.class_descriptor.values()]
        ...
        # text_features_mean = [torch.mean(feature, dim=0) for feature in text_features]
        # ###########################
        text_features_mean = [torch.mean(torch.mul(feature, weight[:,None]), dim=0) for feature,weight in zip(text_features,weights)]
...
class CoOp(TrainerX):
    ...
    def build_model(self):
        ...
        # jsonfile = f"combined_prompts_{cfg.DATASET.NAME}_shot_1-seed_{cfg.SEED}.json"
        jsonfile = cfg.DATASET.class_descriptor_path
        class_descriptor = json.load(open(jsonfile, 'r'))
        # n=3
        # for k,v in class_descriptor.items():
        #     class_descriptor[k] = v[-n:]
    ...
    def after_epoch(self):
        ...
        if meet_checkpoint_freq or last_epoch:
            ...
            self.test()
            if last_epoch:
                print(self.model.prompt_learner.weights)
